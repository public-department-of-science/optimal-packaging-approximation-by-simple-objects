...
namespace hs071_cs
{
    internal class Adapter : BaseAdapter, IDisposable
    {
        /// <summary>
        /// amount of objects
        /// </summary>
        ...
        public Adapter(Data data)
        {
            ...
            * *************************************************************************************/
            ...
            {
                _x_L[3 * i] = -K1 * DiametrSum(radius) + radius[objectCount];
                _x_U[3 * i] = K1 * DiametrSum(radius) - radius[objectCount];

                _x_L[3 * i + 1] = -K1 * DiametrSum(radius) + radius[objectCount];
                _x_U[3 * i + 1] = K1 * DiametrSum(radius) - radius[objectCount];

                _x_L[3 * i + 2] = -K1 * DiametrSum(radius) + radius[objectCount];
                _x_U[3 * i + 2] = K1 * DiametrSum(radius) - radius[objectCount];

                if (max < radius[objectCount])
                {
                    max = radius[objectCount];
                }
            ...
            _nele_jac += 4 * countObjects; // x, y, z , R
            _m += countObjects;

            // (x[i]-x[j])^2 + (y[i]-y[j])^2 + (z[i]-z[j])^2 - (r[i]-r[j])^2 >=0
            _nele_jac += AmountOfIntersectionElement(countObjects);
            _m += countObjects * (countObjects - 1) / 2;
